...
def train(train_dl, val_dl, results):
    ...
    for step, (train_inputs, train_targets) in zip(range(initial_step, TOTAL_TRAINING_STEPS+1),train_dl):
        print(step,end=" ")
        try:
            loss_value, reconstruction_loss, kl_loss = trainval_step(
            ...
            print(f"Step {step}: nelbo={loss_value.item():.4f}, rec={reconstruction_loss.item():.4f}, kl={kl_loss.item():.4f}")
...
for i, axes8 in enumerate(axes32):
    for j, ax in enumerate(axes8):
        ...
        ax.axis('off')
# plt.show()
plt.savefig("./images/generated.jpg", bbox_inches='tight')
